...
from tyger import CMD_HELP, app
from tyger.helper.data import Data
from tyger.helper.inline import cb_wrapper, paginate_help
from tyger import ids as users
...
async def _callbacks(_, callback_query: CallbackQuery):
    ...
    if query == "helper":
        ...
        await app.edit_inline_text(callback_query.inline_message_id, "**ᴄʟᴏꜱᴇ 🗑**")
        return
...
async def on_plug_in_cb(_, callback_query: CallbackQuery):
    ...
    reply_pop_up_alert = (
        ...
        else f"{module_name} Belum ada penjelasannya."
    )
    await app.edit_inline_text(
        callback_query.inline_message_id,
        reply_pop_up_alert,
