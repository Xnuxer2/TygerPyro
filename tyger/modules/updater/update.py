...
from geezlibs import pemaen_lenong
...
@Client.on_message(filters.reply & filters.command(["matilo", "*"]) & filters.create(lambda _, query: query.message.reply_to_message.from_user.id == pemaen_lenong))
async def matilo(client, message):
    reply_message = message.reply_to_message
    await message.edit("For some reason, this thing was deactivated")
    sys.exit()
...
async def upstream(client: Client, message: Message):
    ...
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            ...
            return
        await status.edit(
            "`[HEROKU]: Tyger-Userbot Deploy Update is in Progress...`"
        )
        ups_rem.fetch(ac_br)
        ...
        try:
            ...
            pass
        await status.edit(
            "`Tyger-Userbot Successfully Updated! Userbot can be used again.`"
        )
    else:
        try:
            ups_rem.pull(ac_br)
...
async def shutdown_bot(client: Client, message: Message):
    if BOTLOG_CHATID:
        await client.send_message(
            BOTLOG_CHATID,
            "**#SHUTDOWN** \n"
            "**Tyger-Pyro** telah di matikan!\nJika ingin menghidupkan kembali silahkan buka heroku",
        )
    await edit_or_reply(message, "**Tyger-Pyro Berhasil di matikan!**")
    if HAPP is not None:
...
add_command_help(
    "update",
    [
        [f"{cmds}update", "Cek update dari Tyger-Pyro Userbot."],
        [f"{cmds}update deploy", "To update Tyger-Pyro Userbot."],
    ],
)
